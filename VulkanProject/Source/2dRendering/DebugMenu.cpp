...
void DebugMenu::update(UIManager& uIManager, WorldManager& worldManager, CameraHandler& cameraHandler)
{
    ...
    if (timeFromLastUIRefresh > refreshInterval) {
        ...
        updateUI(uIManager, lastRecordedFps, worldManager, cameraHandler);
    }
}

void DebugMenu::updateUI(UIManager& uIManager, int fps, WorldManager& worldManager, CameraHandler& cameraHandler)
{
    std::cout << "fps = " << fps << "\n";
