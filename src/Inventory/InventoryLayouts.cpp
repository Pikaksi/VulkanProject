...
std::vector<InventorySlotLocation> generategrid10x4InventoryLayout()
{
    ...
    for (int i = 0; i < 40; i++) {
        ...
        glm::vec2 location = {x * (slotSize.x + slotGap.x), y * (slotSize.y + slotGap.y)};
...
std::vector<InventorySlotLocation> generateInput1utput1InventoryLayout()
{
    std::vector<InventorySlotLocation> inventorySlotLocations;
...
std::vector<InventorySlotLocation> generateSlot1InventoryLayout()
{
    std::vector<InventorySlotLocation> inventorySlotLocations;
    inventorySlotLocations.push_back({
        {-0.1f, 0.75f},
...
void generateInventoryLayouts()
{
    ...
    inventoryLayouts.insert(std::make_pair(InventoryLayout::output1Input1, generateInput1utput1InventoryLayout()));
    inventoryLayouts.insert(std::make_pair(InventoryLayout::slot1, generateSlot1InventoryLayout()));
}

std::vector<InventorySlotLocation>& getInventoryLayoutPositions(InventoryLayout inventoryLayout)
{
    assertm(inventoryLayouts.contains(inventoryLayout),
            "Inventory layout is not generated for enum number " << (size_t)inventoryLayout);
