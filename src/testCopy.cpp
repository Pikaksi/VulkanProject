...
int main(int argc, char* argv[])
{
    ...
    for (auto& index : shapes[0].mesh.indices) {
        Vertex v{
            .pos = {attrib.vertices[index.vertex_index * 3],
    ...
    for (auto i = 0; i < textures.size(); i++) {
        ...
        chk(vmaCreateImage(
            allocator, &texImgCI, &texImageAllocCI, &textures[i].image, &textures[i].allocation, nullptr));
        ...
        chk(vmaCreateBuffer(
            allocator, &imgSrcBufferCI, &imgSrcAllocCI, &imgSrcBuffer, &imgSrcAllocation, &imgSrcAllocInfo));
    ...
    while (!quit) {
        ...
        shaderData.projection =
            glm::perspective(glm::radians(45.0f), (float)windowSize.x / (float)windowSize.y, 0.1f, 32.0f);
        ...
        vkCmdBindDescriptorSets(
            cb, VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 0, 1, &descriptorSetTex, 0, nullptr);
