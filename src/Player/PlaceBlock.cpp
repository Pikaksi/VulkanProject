                ...
                ChunkRenderer& chunkRenderer)
{
    ...
    if (blockLocation.x == CHUNK_SIZE - 1) {
        chunkRenderer.renderChunkAgain({chunkLocation.x + 1, chunkLocation.y, chunkLocation.z});
    }
    else if (blockLocation.x == 0) {
        chunkRenderer.renderChunkAgain({chunkLocation.x - 1, chunkLocation.y, chunkLocation.z});
    }
    if (blockLocation.y == CHUNK_SIZE - 1) {
        chunkRenderer.renderChunkAgain({chunkLocation.x, chunkLocation.y + 1, chunkLocation.z});
    }
    else if (blockLocation.y == 0) {
        chunkRenderer.renderChunkAgain({chunkLocation.x, chunkLocation.y - 1, chunkLocation.z});
    }
    if (blockLocation.z == CHUNK_SIZE - 1) {
        chunkRenderer.renderChunkAgain({chunkLocation.x, chunkLocation.y, chunkLocation.z + 1});
    }
    else if (blockLocation.z == 0) {
        chunkRenderer.renderChunkAgain({chunkLocation.x, chunkLocation.y, chunkLocation.z - 1});
    }
    chunkRenderer.renderChunkAgain(chunkLocation);
}
