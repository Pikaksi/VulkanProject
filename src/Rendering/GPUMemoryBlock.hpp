...
struct MemoryLocation
{
    ...
    MemoryLocation(uint64_t startOffset, uint64_t endOffset) : startOffset(startOffset), endOffset(endOffset) {}

    uint64_t getSize() const { return endOffset - startOffset + 1; }
};
...
struct GPUMemoryBlock
{
    ...
    void getVertexData(VkBuffer& outVertexBuffer,
                       std::vector<VkDeviceSize>& vertexOffsets,
                       std::vector<uint32_t>& batchVertexCounts);
    void cleanup(VulkanCoreInfo& vulkanCoreInfo);

    GPUMemoryBlock() {}
    GPUMemoryBlock(VulkanCoreInfo& vulkanCoreInfo,
                   uint32_t sizeofData,
    ...
    template <typename T>
    void
    addData(VulkanCoreInfo& vulkanCoreInfo, VkCommandPool commandPool, uint32_t& memoryLocation, std::vector<T>& data)
    {
  ...
  private:
    template <typename T>
    void copyDataToLocation(VulkanCoreInfo& vulkanCoreInfo,
                            ...
                            std::vector<T>& data)
    {
        if (memoryOffsetStart + size > blockSize) {
            std::cout << "tried to write data at " << memoryOffsetStart << " to " << memoryOffsetStart + size
