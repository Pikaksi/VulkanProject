...
{
	return glm::ivec3(std::floor(x / (float)CHUNK_SIZE), std::floor(y / (float)CHUNK_SIZE), std::floor(z / (float)CHUNK_SIZE));
}

glm::ivec3 getChunkLocation(glm::ivec3 blockLocation)
{
	return glm::ivec3(std::floor(blockLocation.x / (float)CHUNK_SIZE), std::floor(blockLocation.y / (float)CHUNK_SIZE), std::floor(blockLocation.z / (float)CHUNK_SIZE));
}
